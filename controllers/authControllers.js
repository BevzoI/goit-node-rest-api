...
const { ENV_SECRET_KEY, SMTP_HOST, SMTP_PORT, SMTP_SECURE, SMTP_USER, SMTP_PASS } = process.env;
...
export const register = async (req, res, next) => {
  try {
    ...
    if (!email || !password) {
      throw HttpError(400, "Validation error: email and password are required.");
    }
...
export const login = async (req, res, next) => {
  try {
    ...
    const user = await User.findOne({ where: { email } });
    if (!user || !user.verify) {
      throw HttpError(401, "Email or password is wrong, or email is not verified");
    }
    ...
    const token = jwt.sign({ id: user.id }, SECRET_KEY, { expiresIn: "5 days" });
